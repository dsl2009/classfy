...
def run(trainr,testdr, name,cls_num,idx):
    ...
    #model.load_state_dict(torch.load('/home/dsl/all_check/inceptionv4-8e4777a0.pth'), strict=False)
    #model.last_linear = nn.Linear(1536, cls_num)
    model.fc1 = nn.Linear(1536, 2)
    ...
    def train():
        ...
        for (data, target) in progress(data_loader):
            ...
            loss = focal_loss(output, target) + centerloss(target, f1)*0.5
            loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    train_dr = '/media/dsl/20d6b919-92e1-4489-b2be-a092290668e4/dsl/train/1009'
    test_dr = '/media/dsl/20d6b919-92e1-4489-b2be-a092290668e4/dsl/valid/step1'
    run(train_dr, test_dr, 'luntai', 2, '1009_iv_step1')